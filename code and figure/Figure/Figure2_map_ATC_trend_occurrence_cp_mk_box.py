...
for i, model in enumerate(models):
    ...
    ATC_slope_global_raw=ATC_slope[lat > -60, :][~np.isnan(ATC_slope[lat > -60, :])]
    ATC_slope_NH_raw    =ATC_slope[lat > 0, :][~np.isnan(ATC_slope[lat > 0, :])]
    ATC_slope_SH_raw    =ATC_slope[(lat<0)&(lat>-60),:][~np.isnan(ATC_slope[(lat<0)&(lat>-60),:])]
    ...
    global_ATC_frac_d_raw  =ATC_frac_d[lat > -60, :][~np.isnan(ATC_frac_d[lat > -60, :])]
    NH_ATC_frac_d_raw      =ATC_frac_d[lat > 0, :][~np.isnan(ATC_frac_d[lat > 0, :])]
    SH_ATC_frac_d_raw      =ATC_frac_d[(lat<0)&(lat>-60),:][~np.isnan(ATC_frac_d[(lat<0)&(lat>-60),:])]
...
#    ATC_slope_global_raw = ATC_slope_global_raw[ATC_slope_global_raw != 0]
#    ATC_slope_NH_raw = ATC_slope_NH_raw[ATC_slope_NH_raw != 0]
#    ATC_slope_SH_raw = ATC_slope_SH_raw[ATC_slope_SH_raw != 0]
#    
#    global_ATC_frac_d_raw = global_ATC_frac_d_raw[global_ATC_frac_d_raw != 0]
#    NH_ATC_frac_d_raw = NH_ATC_frac_d_raw[NH_ATC_frac_d_raw != 0]
#    SH_ATC_frac_d_raw = SH_ATC_frac_d_raw[SH_ATC_frac_d_raw != 0]


    all_data.extend([
            {'Model': model, 'Region': 'Globe', 'Type': 'ATC_slope_sp', 'Value': ATC_slope_global_raw,'error': ATC_slope_global_std},
            {'Model': model, 'Region': 'NH'    , 'Type': 'ATC_slope_sp', 'Value': ATC_slope_NH_raw,'error': ATC_slope_NH_std},
            {'Model': model, 'Region': 'SH'    , 'Type': 'ATC_slope_sp', 'Value': ATC_slope_SH_raw,'error': ATC_slope_SH_std},

            {'Model': model, 'Region': 'Globe', 'Type': 'ATC_frac_d', 'Value': global_ATC_frac_d_raw,'error': global_ATC_frac_d_std},
            {'Model': model, 'Region': 'NH'    , 'Type': 'ATC_frac_d', 'Value': NH_ATC_frac_d_raw,'error': NH_ATC_frac_d_std},
            {'Model': model, 'Region': 'SH'    , 'Type': 'ATC_frac_d', 'Value': SH_ATC_frac_d_raw,'error': SH_ATC_frac_d_std},
        ])
...
width = 0.5
print(ATC_data['Value'].values)

for reg in region:
    data = ATC_data[ATC_data['Region']==reg]['Value'].iloc[0]
    d = data[~np.isnan(data)]
    print(f"CRUJRA {reg}: Q1={np.percentile(d,25):.4f} Q2={np.percentile(d,50):.4f} Q3={np.percentile(d,75):.4f} zeros={np.mean(d==0)*100:.0f}%")
...
ax_inset1.boxplot(ATC_data['Value'].values, positions=x, tick_labels=region, vert=False,  showfliers=False, patch_artist=True,
                  boxprops=dict(facecolor='#F2E6D8', edgecolor='black', linewidth=0.5), 
                  whiskerprops=dict(linewidth=0.5), capprops=dict(linewidth=0.5), medianprops=dict(linewidth=0.5,color='red'),
                  showmeans=True, meanline=True, meanprops=dict(linestyle=':', color='red', linewidth=0.5))
...
ax_inset2.boxplot(ATCc_frac_data['Value'].values, positions=x, tick_labels=region, vert=False,  showfliers=False, patch_artist=True,
                  boxprops=dict(facecolor='#F2E6D8', edgecolor='black', linewidth=0.5), 
                  whiskerprops=dict(linewidth=0.5), capprops=dict(linewidth=0.5), medianprops=dict(linewidth=0.5,color='red'),
                  showmeans=True, meanline=True, meanprops=dict(linestyle=':', color='red', linewidth=0.5)) 
...
total = np.sum(~np.isnan(ERA5_ATC_slope))
points = np.sum((mask == 1) & (~np.isnan(ERA5_ATC_slope)))
ratio = points / total * 100
print(f"ERA5 显著占比: {ratio:.2f}% ({points}/{total})")
...
for reg in region:
    data = ATC_data[ATC_data['Region']==reg]['Value'].iloc[0]
    d = data[~np.isnan(data)]
    print(f"ERA5 {reg}: Q1={np.percentile(d,25):.4f} Q2={np.percentile(d,50):.4f} Q3={np.percentile(d,75):.4f} zeros={np.mean(d==0)*100:.0f}%")
...
ax_inset3.boxplot(ATC_data['Value'].values, positions=x, tick_labels=region, vert=False,  showfliers=False, patch_artist=True,
                  boxprops=dict(facecolor='#F2E6D8', edgecolor='black', linewidth=0.5), 
                  whiskerprops=dict(linewidth=0.5), capprops=dict(linewidth=0.5), medianprops=dict(linewidth=0.5,color='red'),
                  showmeans=True, meanline=True, meanprops=dict(linestyle=':', color='red', linewidth=0.5))
...
ax_inset4.boxplot(ATCc_frac_data['Value'].values, positions=x, tick_labels=region, vert=False,  showfliers=False, patch_artist=True,
                  boxprops=dict(facecolor='#F2E6D8', edgecolor='black', linewidth=0.5), 
                  whiskerprops=dict(linewidth=0.5), capprops=dict(linewidth=0.5), medianprops=dict(linewidth=0.5,color='red'),
                  showmeans=True, meanline=True, meanprops=dict(linestyle=':', color='red', linewidth=0.5)) 
...
ratio = points / total * 100
print(f"CMIP6 显著占比: {ratio:.2f}% ({points}/{total})")
...
for reg in region:
    data = ATC_data[ATC_data['Region']==reg]['Value'].iloc[0]
    d = data[~np.isnan(data)]
    print(f"CMIP6 {reg}: Q1={np.percentile(d,25):.4f} Q2={np.percentile(d,50):.4f} Q3={np.percentile(d,75):.4f} zeros={np.mean(d==0)*100:.0f}%")
...
ax_inset5.boxplot(ATC_data['Value'].values, positions=x, tick_labels=region, vert=False,  showfliers=False, patch_artist=True,
                  boxprops=dict(facecolor='#F2E6D8', edgecolor='black', linewidth=0.5), 
                  whiskerprops=dict(linewidth=0.5), capprops=dict(linewidth=0.5), medianprops=dict(linewidth=0.5,color='red'),
                  showmeans=True, meanline=True, meanprops=dict(linestyle=':', color='red', linewidth=0.5)) 
...
ax_inset6.boxplot(ATCc_frac_data['Value'].values, positions=x, tick_labels=region, vert=False,  showfliers=False, patch_artist=True,
                  boxprops=dict(facecolor='#F2E6D8', edgecolor='black', linewidth=0.5), 
                  whiskerprops=dict(linewidth=0.5), capprops=dict(linewidth=0.5), medianprops=dict(linewidth=0.5,color='red'),
                  showmeans=True, meanline=True, meanprops=dict(linestyle=':', color='red', linewidth=0.5))  
