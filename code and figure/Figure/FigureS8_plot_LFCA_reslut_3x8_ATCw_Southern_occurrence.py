...
neg_colors = base_cmap(np.linspace(0, 0.5, int(256*neg_ratio), endpoint=False))
pos_colors = base_cmap(np.linspace(0.5, 1, int(256*(1-neg_ratio)), endpoint=True))

# 合并成新的colormap
new_cmap = ListedColormap(np.vstack((neg_colors, pos_colors)))

# 归一化到数据范围
norm = BoundaryNorm(np.linspace(vmin, vmax, 256), new_cmap.N)
...
for i, model in enumerate(models):
    ...
    if model=='CRUJRA':
        ...
        black='black'
        zorder=1
    lfps_up = dataset['lfps_up'][0,:]*364
    lfcs_up = dataset['lfcs_up'][0,:]
    lfps_up = lfps_up*(np.nanmean(lfcs_up[-120:])-np.nanmean(lfcs_up[:120])) / ((len(lfcs_up)-120) /12)
    print(len(lfcs_up))
    time = [time + (t / 12.0) for t in range(len(lfcs_up))]
    
    #im_lfps_up_CRUJRA = axes[i].pcolormesh(dataset["lon"][:], dataset["lat"][:], lfps_up, transform=ccrs.PlateCarree(), cmap='coolwarm',vmin=vmin,vmax=vmax,)
    im_lfps_up_CRUJRA = axes[i].pcolormesh(dataset["lon"][:], dataset["lat"][:], lfps_up, transform=ccrs.PlateCarree(), cmap=base_cmap,norm=norm)
    ...
    sns.lineplot(x=time, y=lfcs_up,  ax=axes[22], color=light, alpha=0.4, lw=0.5,zorder=zorder)
    smoothed_values = uniform_filter1d(lfcs_up, size=10, mode='nearest')
...
#cbar.set_label(r'change in ATC$_w$ (yr$^{-1}$)', labelpad=-40,  horizontalalignment='center')
cbar.ax.set_xlabel(r'change in ATCw occurrence (yr⁻²)', labelpad=-40, ha='left', x=-0.22)
